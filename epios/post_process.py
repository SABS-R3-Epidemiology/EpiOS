...
class PostProcess():
    ...
    def __init__(self, demo_data: pd.DataFrame, time_data: pd.DataFrame, inf_data: pd.DataFrame = None, model=None):
        self.demo_data = demo_data
        self.time_data = time_data
        self.inf_data = inf_data
        self.model = model
        self.predict = self.Prediction(demo_data=demo_data, time_data=time_data, inf_data=inf_data, model=model)
    ...
    class Prediction():
        ...
        def _compare(self, time_sample, gen_plot=False, scale_method: str = 'proportional', saving_path_compare=None, get_true_result=False):
            ...
            est_rate = [self.result[i][0] for i in range(len(self.result))]
            act_rate = [self.result[i][1] for i in range(len(self.result))]
            ...
            if non_responder:  # For non-responders enabled
                ...
                for i in range(len(time_sample)):  # Sample again at each time point
                    ...
                    X = SamplingMaker(non_resp_rate=non_resp_rate, keep_track=True, data=self.time_data,
                                      ...
                                      inf_data=self.inf_data, model=self.model)
                ...
                if sample_strategy == 'Same':  # Do not change people sampled at each sample time point
                    ...
                    for i in range(len(time_sample)):
                        ...
                        TP = ite.iloc[i].value_counts().get('[Positive, True]', 0) / len(people)
                        FP = ite.iloc[i].value_counts().get('[Positive, False]', 0) / len(people)
                        TN = ite.iloc[i].value_counts().get('[Negative, True]', 0) / len(people)
                        FN = ite.iloc[i].value_counts().get('[Negative, False]', 0) / len(people)

                        #infected_rate.append([est_rate, act_rate])

                        infected_rate.append([TP, FP, TN, FN])
                    ...
                    for i in range(len(time_sample)):  # Sample at each sample time points
                        ...
                        TP = ite.iloc[0].value_counts().get('[Positive, True]', 0) / len(people)
                        FP = ite.iloc[0].value_counts().get('[Positive, False]', 0) / len(people)
                        TN = ite.iloc[0].value_counts().get('[Negative, True]', 0) / len(people)
                        FN = ite.iloc[0].value_counts().get('[Negative, False]', 0) / len(people)
