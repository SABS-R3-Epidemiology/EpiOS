...
class SamplerRegion(Sampler):
    ...
    def multinomial_draw(self, n: int, prob: list):
        ...
        prob(list): list of floats, sum to 1, length should be number of region groups

        Output:
        res(list): a list of integers indicating the number of samples from each region group
        record_cap_region(list): a list of caps for each region group
    ...
    def sample(self, sample_size: int, additional_sample: list = None,
        ...
        household_criterion(boolean): Turn on or off the household criterion
        household_threshold(int): The maximum number of people sampled from one household
    ...
    def additional_nonresponder(self, nonRespID: list, sampling_percentage=0.1, proportion=0.01, threshold=None):
        ...
        sampling_percentage(float, between 0 and 1): The proportion of additional samples
                                                     taken from a specific regional group
        proportion(float, between 0 and 1): The proportion of total groups to be sampled additionally
        threshold(NoneType or Int): The lowest number of regional groups to be sampled additionally
        ...
        Output:
        additional_sample(list of 2D, with dimension num_age_group):
                        A list containing how many additional samples we would like to draw
                        from each region group
